...
The \texttt{/deanonymize-known} endpoint is the server-facing operation. It receives an anonymised string and a \texttt{contextId}, retrieves the mapping from Redis via a \texttt{Pop} operation (atomic read-and-delete), and restores original values through string substitution. The design intent is one-time use: a \texttt{Pop} consumes the mapping so it does not linger in Redis past the point it was actually needed, limiting the exposure window for the mapping to the minimum tool execution requires. That intent assumes one deanonymisation call per context. It does not always hold: Section~\ref{sec:usecases_validation} finds cases where a single \texttt{ask\_antares} invocation drives more than one call to \texttt{/deanonymize-known} against the same \texttt{contextId}, and in the cases measured there, each of those calls returns a correct result. This chapter does not establish how the store handles a second call against an already-consumed context; Section~\ref{sec:usecases_validation} reports the observed outcome, not the mechanism behind it.
